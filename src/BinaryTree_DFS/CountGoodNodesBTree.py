"""
1448. Count Good Nodes in Binary Tree

Given a binary tree root, a node X in the tree is named good if in the path from root to X there are no nodes with a value greater than X.

Return the number of good nodes in the binary tree.

Example 1:

Input: root = [3,1,4,3,null,1,5]
Output: 4
Explanation: Nodes in blue are good.
Root Node (3) is always a good node.
Node 4 -> (3,4) is the maximum value in the path starting from the root.
Node 5 -> (3,4,5) is the maximum value in the path
Node 3 -> (3,1,3) is the maximum value in the path.
Example 2:

Input: root = [3,3,null,4,2]
Output: 3
Explanation: Node 2 -> (3, 3, 2) is not good, because "3" is higher than it.
Example 3:

Input: root = [1]
Output: 1
Explanation: Root is considered as good.
"""
from src.helper.create_binary_tree import list_to_tree, print_tree
# Definition for a binary tree node.
class TreeNode(object):
    def __init__(self, val=0, left=None, right=None):
        self.val = val
        self.left = left
        self.right = right
class Solution(object):
    def goodNodes(self, root):
        """
        using DFS recursively
        :type root: TreeNode
        :rtype: int
        """
        def dfs(node, max_val):
            if not node:
                return 0
            count = 1 if node.val >= max_val else 0
            count += dfs(node.left, max(max_val, node.val))
            count += dfs(node.right, max(max_val, node.val))
            return count
        return dfs(root, float("-inf"))

    def goodNodes2(self, root):
        """
        using DFS Iteratively
        :param root:
        :return:
        """
        if not root:
            return 0
        stack = []
        good_nodes = 0
        stack.append((root,float("-inf")))
        while stack:
            node, val = stack.pop()
            if node.val >= val:
                good_nodes += 1
            if node.left:
                stack.append((node.left,max(node.val, val)))
            if node.right:
                stack.append((node.right, max(node.val, val)))
        return good_nodes

    def goodNodes3(self, root):
        """
        using BFS
        :param root:
        :return:
        """
        if not root:
            return 0
        from collections import deque
        queue = deque()
        queue.append((root, float("-inf")))
        good_nodes = 0
        while queue:
            node, val = queue.popleft()
            if node.val >= val:
                good_nodes += 1
            if node.left:
                queue.append((node.left, max(node.val, val)))
            if node.right:
                queue.append((node.right, max(node.val, val)))
        return good_nodes

sol = Solution()
root = [3,1,4,3,None,1,5]
# root = [3,3,None,4,2]
# root = [1]
print(sol.goodNodes(list_to_tree(root)))
print(sol.goodNodes2(list_to_tree(root)))
print(sol.goodNodes3(list_to_tree(root)))
